...
import type { Message, User } from "~/api/model";
...
async function deactivateUser(user: User) {
  await useDeleteApi<Message>(`/api/v1/user/${user.id}`, {
    onResponse({ response }) {
...
async function activateUser(user: User) {
  await usePutApi<Message>(`/api/v1/user/${user.id}/activate`, {
    onResponse({ response }) {
      if (response.status === 200) {
        useToast().success(response._data?.message);
        refresh();
...
function editUser(user: User) {
  if (userEditMap.value && user.id && userEditMap.value.get(user.id)) {
    userEditMap.value.set(user.id, false);
  }
  else if (userEditMap.value && user.id) {
    userEditMap.value.set(user.id, true);
  }
  refresh();
}
              ...
              <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                  <tr>
                    <th
                      scope="col"
                      class="w-8 px-8 py-3.5 text-left text-sm font-normal text-gray-500 dark:text-gray-400"
                    />
                ...
                >
                  ...
                  >
                    <td class="whitespace-nowrap py-2 pl-5 text-sm font-medium">
                      <div
                        class="mx-1 size-8 overflow-hidden rounded-full border border-gray-400"
                      >
                        <img
                          :src="user.picture"
                    ...
                    <td class="whitespace-nowrap p-4 text-sm font-medium">
                      ...
                      >
                        ...
                        <button
                          v-if="user.active"
                          class="items-center justify-center gap-x-2 rounded-lg bg-red-500 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 hover:bg-red-600 sm:w-auto dark:bg-red-600 dark:hover:bg-red-500"
                        ...
                        <button
                          v-else
                          class="items-center justify-center gap-x-2 rounded-lg bg-blue-500 px-5 py-2 text-sm tracking-wide text-white transition-colors duration-200 hover:bg-blue-600 sm:w-auto dark:bg-blue-600 dark:hover:bg-blue-500"
